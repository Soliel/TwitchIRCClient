...
namespace TwitchIRCClient
{
    ...
    public delegate void RawMessageReceivedEvent(string message);
    public class TwitchClient : MonoBehaviour
    {
        public event MessageReceivedEvent OnMessageReceived;
        public event RawMessageReceivedEvent OnRawMessageReceived;
        private string oauthToken;
        private string userName;
        private string channelName;
        private string serverIP = "irc.chat.twitch.tv";
        private int serverPort = 6667;
        public void SetCredentials(string oauthToken, string userName, string channelName)
        {
        ...
        public bool InitializeIRC()
        {
            ...
            outputStream.WriteLine("NICK " + (oauthToken != string.Empty ? userName : "justinfan" + UnityEngine.Random.Range(0, 99999).ToString().PadLeft(5, '0')));
            outputStream.Flush();
        ...
        private void InputThread(TextReader inputStream, NetworkStream networkStream)
        {
            while (enabled)
            {
                ...
                OnRawMessageReceived?.Invoke(buffer);
            }
        }
        ...
        public void Update()
        {
            if(enabled == false && Input.GetKeyDown(KeyCode.F6))
            {
                InitializeIRC();
            }
            lock (chatBuffer)
